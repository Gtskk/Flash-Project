package 
{
	...
	public class Main extends Sprite 
	{
		...
		private function init(e:Event = null):void 
		{
			...
			mainyuanjian.width=stage.stageWidth - 110;
			mainyuanjian.height=stage.stageHeight - 50;

			removeEventListener(Event.ADDED_TO_STAGE, init);
			loader = new URLLoader();
			loader.addEventListener(Event.COMPLETE, onLoadDataBefore);
			loader.load(new URLRequest(loaderInfo.parameters.filename || "tuoxiao.xml"));
		...
		private function onStageResize(e:Event):void {
  			...
  			background.width=w;
  			background.height=h;
			mainyuanjian.width=w - 110;
			mainyuanjian.height=h - 50;
		}
		...
		private function onLoadDataBefore(e:Event):void{
			...
			var scrurl:String = datas.scrurl;
			...
			for(var j:int = 0;j<componsList.length();j++){
				...
				//定义SCR反应器链接
				if(componsList[j].name == 'SCR反应器'){
					var componICon:ScrButton = new ScrButton();
					mainyuanjian.dongtai.addChild(componICon);
					
					componICon.x = componsList[j].x_pos -20;
					componICon.y = componsList[j].y_pos - 20;
					
					componICon.addEventListener(MouseEvent.CLICK, function(e:MouseEvent){navigateToURL(new URLRequest(scrurl));});
				}else{
			...
			for(var i:int = 0; i < paramsList.length();i++){
				...
				paraTf.align = TextFormatAlign.LEFT;
				paraTf.size = 11;
